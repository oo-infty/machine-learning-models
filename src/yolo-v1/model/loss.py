...
class YoloLoss(Module):
    ...
    def forward(self, output: Tensor, target: Tensor) -> Tensor:
        (
            ...
            output_confidence,
            output_bounding_box,
        ) = self.context.split_tensor(output)
        (
            target_classes,
            target_confidence,
            target_bounding_box,
        ) = self.context.split_tensor(target)
        converted_output_bounding_box = self.context.convert_bounding_box(
            output_bounding_box
        )
        converted_target_bounding_box = self.context.convert_bounding_box(
            target_bounding_box
        )
        ...
        bounding_box_id = (
            torch.arange(shape[3]).repeat([*shape[0:3], 1]).to(self.device)
        )
        optimal_bounding_box_id = (
            output_target_iou.argmax(3)
            .repeat_interleave(shape[3])
            .reshape_as(bounding_box_id)
        )

        indicator_obj_grid = torch.all(target_confidence[:, :, :] == 1, dim=3)
        indicator_obj = (target_confidence != 0) & (
            optimal_bounding_box_id == bounding_box_id
        )
