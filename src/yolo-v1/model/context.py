...
class YoloContext:
    def __init__(
        ...
        self.offset_x = (
            self.offset_x.reshape(self.size, self.size, 1)
            .repeat_interleave(self.bounding_box, dim=2)
            .to(device)
        )

        self.offset_y = (
            self.offset_y.reshape(self.size, self.size, 1)
            .repeat_interleave(self.bounding_box, dim=2)
            .to(device)
        )
        ...
        self.network = Sequential(
            backbone_net, YoloExtractor(), YoloDetector(size, bounding_box, classes)
        ).to(device)
    ...
    def split_tensor(self, tensor: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        ...
        tensor = tensor.reshape(
            -1, self.size, self.size, self.classes + 5 * self.bounding_box
        )
    ...
    def convert_bounding_box(self, bounding_box: Tensor) -> Tensor:
        ...
        return torchvision.ops.box_convert(
            new_bounding_box.reshape(-1, 4), "cxcywh", "xyxy"
        ).reshape(-1, self.size, self.size, self.bounding_box, 4)

    def select_class(
        self, classes: Tensor, confidence: Tensor
    ) -> tuple[Tensor, Tensor]:
        ...
        reshaped_confidences = confidence.reshape(
            -1, self.size, self.size, self.bounding_box, 1
        )
        max_class_confidence, indices = (reshaped_confidences * reshaped_classes).max(
            dim=4
        )
        return max_class_confidence, indices

    def filter_bounding_box(
        self, bounding_box: Tensor, class_confidence: Tensor, classes: Tensor
        ...
        indices = torchvision.ops.batched_nms(
            new_bounding_box, new_class_confidence, batch_id, self.iou_threshold
        )
